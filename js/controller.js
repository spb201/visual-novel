...
var graphLayout = function(_name) {
  setInterval(function() {
    _cy.makeLayout({name: _name}).run()
  }, 1000);
};
...
var ngApp = angular.module("ngApp", ['ngRoute', "firebase", "infinite-scroll"])
  ...
  .controller("makerController", ["$scope", "$firebaseObject", "$firebaseAuth", "$routeParams", function($scope, $firebaseObject, $firebaseAuth, $routeParams) {
                ...
                if ($scope._q.nodes[i].ways_ids[j]) {
                  qedges.push({
                    data: {
                      ...
                      weight: j,
                      source: '' + i,
                      target: '' + $scope._q.nodes[i].ways_ids[j]
                    }
                  });
                }
              }
    ...
    $scope.$watch(
      function($scope) {
        return $scope._q.nodes ? $scope._q.nodes.map(function(node) {
          return node.ways_ids;
        }) : null;
      },
      function() {
        if (typeof(cytoscape) !== 'undefined') $scope.renewGraph();
      },
      true
    );
